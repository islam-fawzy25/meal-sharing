...
// get available Reservations " Today " For Single Meal 
  ...
  FROM
   ...
   GROUP BY  reservations.created_date;
     `)
      ...
      return  response.status(200).send(dailyReservationsForsingleMeals)
      });
  } catch (error) {
    response.status(500).send({ error: "Internal Server Error." });
  }
...
// get available Reservations For Single Meal in general
